...
1. **Postgres:** same instance as the `maayan_recipes` project, new database within it (not a separate instance). **Corrected 2026-08-22:** originally recorded as "same instance as `photography`" — Adi corrected this after that instance's connection pattern had already been investigated; `photography` uses Neon via Replit, but the actual target is `maayan_recipes`, which connects directly to a **Google Cloud SQL** Postgres instance (`server/database.js`: raw `pg` driver, host `34.132.167.99:5432`). Don't reuse the password found in that file without Adi confirming it directly (dev-plan.md §11). **Corrected again, same day:** this hit a dead end - this machine's public IP isn't in that Cloud SQL instance's authorized-networks allowlist, and raw-TCP connections timed out. **Final decision: a dedicated Neon serverless Postgres project**, separate from both `maayan_recipes` and `photography` - no IP-allowlist step, free tier is enough for this single-user app. Live as of 2026-08-22 (see `docs/PROJECT_STATUS.md` and the `postgres-instance` memory for the full reasoning).
